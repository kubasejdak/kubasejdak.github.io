---
title: "Variadic functions - Part 1: va_args and friends"
date: "2019-05-11"
slug: "variadic-functions-va-args"
category: "C++"
tags:
  - "functions"
  - "standard library"
series: "Variadic functions"
---

In the series:
- [Variadic functions - Part 1: va_args and friends](/variadic-functions-va-args)
...
trailing ellipsis. It looks like an optional hint to the function implementer, but in fact it is required by the
[C standard](http://www.open-std.org/jtc1/sc22/wg14/www/docs/n2346.pdf) ([C++refers](http://eel.is/c++draft/cstdarg.syn)
to C on this matter).
...
- pointer to the stack ([old Apple kernel implementation](https://opensource.apple.com/source/xnu/xnu-792.13.8/EXTERNAL_HEADERS/stdarg.h)),
...
Feel free to play and inspect this example on different platforms and compiler using
[Compiler Explorer](https://godbolt.org/z/nD5OAZ).
...
It is really important to remember, because it's not obvious. For example: if you pass the `char` or `bool`, you still
have to extract an `int`! Here is an example:

```cpp {numberLines: true}{8}
#include <cstdarg>
...
- function accepting a container of arguments (e.g. `std::vector`, `std::array` or `std::initializer_list`) - this
approach can be used only if arguments share a common type. Of course, we can use the type erasure technique
(`std::any`, `std::variant`), but it will add another layer to the solution making it harder to understand,
...
recommendations to avoid it (e.g. [C++ Core Guidelines](https://isocpp.github.io/CppCoreGuidelines/CppCoreGuidelines#f55-dont-use-va_arg-arguments)
and [SEI CERT C++ Coding Standard](https://wiki.sei.cmu.edu/confluence/display/cplusplus/DCL50-CPP.+Do+not+define+a+C-style+variadic+function)),
